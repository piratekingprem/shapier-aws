...
exports.store = async (file, params) => {
  ...
  try {
    const banners = await db.query(
      `INSERT INTO banners_for_others(banner_image,banner_category) VALUES(?,?)`,
...
exports.get_by_id = async (id) => {
  let message = "Something went wrong",
    ...
    data = [];
  try {
    const banner = await db.query(`SELECT * from banners_for_others WHERE id=${id}`, []);
...
exports.get = async () => {
  let message = "Something went wrong",
    ...
    data = [];
  try {
    const banner = await db.query(`SELECT * from banners_for_others`, []);
    (message = "No banners found"), (code = 400), (data = []);
    if (banner.length) {
      (message = "Succesfully fetched banners"), (code = 200), (data = banner);
    }
...
exports.get_by_banner_category = async (banner_category) => {
  ...
  try {
    const banner = await db.query(
      `SELECT * from banners_for_others WHERE banner_category='${banner_category}'`,
      []
    );
    (message = "No banners found"), (code = 400), (data = []);
    if (banner.length) {
